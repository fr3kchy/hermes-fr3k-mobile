...
class HybridSearcher:
    ...
    def _fit_bm25(self):
        """训练 BM25 模型"""
        if self._corpus and BM25Okapi is not None:
            self._bm25 = BM25Okapi(self._corpus)
            self._fitted = True
            logger.info(f"✅ BM25 index fitted with {len(self.data)} documents")
        elif self._corpus:
            logger.warning("rank_bm25 not installed; BM25 index disabled")
    ...
    def _fit_vector(self):
        ...
        if SentenceTransformer is None or cosine_similarity is None:
            logger.warning("vector-search dependencies missing; vector index disabled")
            self._vector_fitted = False
            return
