...
try:
    ...
    class DuckDuckGoTools:  # type: ignore[override]
        def duckduckgo_search(self, query: str, max_results: int = 5) -> List[Dict[str, str]]:
            logger.warning("agno not installed; DuckDuckGoTools fallback returning no results")
            return []
...
try:
    ...
    class BaiduSearchTools:  # type: ignore[override]
        def baidu_search(self, query: str, max_results: int = 5) -> List[Dict[str, str]]:
            logger.warning("agno not installed; BaiduSearchTools fallback returning no results")
            return []
