...
try:
    ...
    Kline = Line = Bar = Grid = Radar = Graph = Any  # type: ignore[misc,assignment]
    opts = None
    ThemeType = None
...
class VisualizerTools:
    ...
    def __init__(self):
        self.pyecharts_available = opts is not None

    @staticmethod
    def _require_pyecharts() -> None:
        if opts is None:
            raise RuntimeError("pyecharts is required for chart rendering but is not installed")
    ...
    def generate_stock_chart(
        ...
        """
        VisualizerTools._require_pyecharts()
        if df.empty:
            return None
