...
SUPABASE_API_KEY="${SUPABASE_SERVICE_KEY:-${SUPABASE_SECRET_KEY:-${SUPABASE_SERVICE_ROLE_KEY:-}}}"

if [[ -z "${SUPABASE_API_KEY:-}" ]]; then
    echo "Error: set SUPABASE_SERVICE_KEY, SUPABASE_SECRET_KEY, or SUPABASE_SERVICE_ROLE_KEY" >&2
    exit 1
fi
...
cmd_tables() {
    ...
    if [[ -z "$response" ]] || [[ "$response" == *"permission denied"* ]]; then
        ...
        curl -s -I -X GET "${REST_URL}/" \
            -H "apikey: ${SUPABASE_API_KEY}" \
            -H "Authorization: Bearer ${SUPABASE_API_KEY}" 2>/dev/null \
            | grep -i "x-" | head -20
    else
